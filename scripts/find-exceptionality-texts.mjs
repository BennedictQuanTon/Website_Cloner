...
// The main section is Exceptionalities
const index = html.indexOf('Exceptionalities');
if (index !== -1) {
  const sectionHtml = html.substring(index, index + 35000);
  
  // Find all text inside paragraphs <p>
  const pMatches = [...sectionHtml.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/g)];
  console.log('--- ALL PARAGRAPHS IN EXCEPTIONALITIES SECTION ---');
  ...
  console.log('\n--- ALL HEADINGS IN EXCEPTIONALITIES SECTION ---');
